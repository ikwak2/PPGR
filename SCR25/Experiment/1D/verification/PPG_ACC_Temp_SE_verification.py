...
print(f"⚙️  25s Stabilization Drop Applied | Users: {NUM_USERS} | Dropout: {DROPOUT_RATE}")
...
class VerificationModel(nn.Module):
    def __init__(self, num_users=16, d_model=256, dropout_p=0.2):
        super().__init__()
        ...
        # 🔥 Fusion Projector 중간에도 Dropout 추가
    ...
    def forward(self, x_p, x_t, x_a, labels=None):
        ...
        # 세 가지 모달리티 합친 후 Dropout 적용
        concat_feat = torch.cat([z_p, z_t, z_a], dim=1)
        concat_feat = self.dropout(concat_feat)
        
        z_norm = F.normalize(self.fusion_projector(concat_feat), p=2, dim=1)
        
        if labels is not None: 
            return self.arcface(z_norm, labels), z_norm
        return None, z_norm
...
if __name__ == "__main__":
    ...
    # Dropout 파라미터 전달
    model = VerificationModel(NUM_USERS, D_MODEL, dropout_p=DROPOUT_RATE).to(DEVICE)
    ...
    for epoch in range(EPOCHS):
        ...
        for (p, t, a), lbl, _ in tqdm(train_loader, desc=f"Ep {epoch+1}/{EPOCHS}"):
            ...
            # arc_logits는 loss 계산용(margin 포함), z_norm은 정확도 계산용(순수 임베딩)
            arc_logits, z_norm = model(p, t, a, lbl)
            loss = criterion(arc_logits, lbl)
            loss.backward()
            optimizer.step()
            
            epoch_loss += loss.item()
            
            # 🔥 [수정] Train Accuracy 버그 수정: Margin이 포함되지 않은 순수 Logit으로 정확도 산출
            with torch.no_grad():
                weights_norm = F.normalize(model.arcface.weight, p=2, dim=1)
                clean_logits = torch.mm(z_norm, weights_norm.t()) * S
                _, pred = torch.max(clean_logits, 1)
