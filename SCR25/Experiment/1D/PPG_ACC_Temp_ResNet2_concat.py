...
print(f"🚀 [Start] ResNet2 + Concat Fusion + Visualization Ready", flush=True)
print(f"⚙️  Device: {DEVICE} | Epochs: {EPOCHS}", flush=True)
...
# ==========================================
# 3. 모델 아키텍처: ResNet2 + Concat Fusion
# ==========================================
...
class ResNet2Encoder(nn.Module):
    def __init__(self, in_channels=1, d_model=256):
        ...
        # Grad-CAM을 위해 마지막 레이어를 속성으로 접근 가능하게 함
...
def visualize_tsne(embeddings, labels, save_path='tsne_result.png', max_samples=2000):
    ...
    if len(embeddings) > max_samples:
        ...
        print(f"   (속도를 위해 {max_samples}개 샘플만 사용합니다)")
    ...
    plt.title('t-SNE Visualization of User Embeddings (128D -> 2D)', fontsize=15)
...
class GradCAM1D:
    ...
    def __call__(self, x_ppg, x_temp, x_acc, target_class_idx):
        ...
        for name, grads in self.gradients.items():
            acts = self.activations[name] # (B, C, L_out)
            
            # Global Average Pooling of Gradients (Weights)
            weights = torch.mean(grads, dim=2, keepdim=True) # (B, C, 1)
            
            # Weighted sum of activations
            cam = torch.sum(weights * acts, dim=1, keepdim=True) # (B, 1, L_out)
            cam = F.relu(cam) # ReLU
            
            # Upsample to original input length (300)
...
def visualize_gradcam_samples(model, data_loader, device, save_path='gradcam_analysis.png', num_samples=3):
    ...
    # 각 인코더의 마지막 ResNet 레이어를 타겟으로 설정
    target_layers = {
    ...
    for i in range(num_samples):
        ...
        # 3. Acc Plot (X축만 대표로 표시)
        plt.subplot(num_samples, 3, i*3 + 3)
        acc_signal_x = acc.squeeze().cpu().numpy()[0] # X축만
...
if __name__ == "__main__":
    ...
    # 4. Grad-CAM 시각화 저장
    # (새로운 DataLoader 인스턴스를 만들어 배치 단위로 가져옴)
