...
def generate_cwt_image(sig, fs, f_min, f_max, num_scales, img_size):
    ...
    # 4. fixed scaling (clipping)
    cwt_log = np.clip(cwt_log, 0.0, 6.0)
    cwt_norm = cwt_log / 6.0
...
def preprocess_all():
    ...
    for filepath in tqdm(file_list, desc="Processing Users"):
        ...
        for segment_df in df_segments:
            ...
            # 정규화
            #temp_norm = (raw_temp - 25.0) / (40.0 - 25.0)   # 뒤에 window단위로 다시 정규화 함 
            acc_mean = np.mean(raw_acc, axis=0)
            acc_std = np.std(raw_acc, axis=0) + 1e-6
            acc_norm = (raw_acc - acc_mean) / acc_std

            num_windows = (len(ppg_filtered) - WINDOW_SIZE) // STRIDE + 1
            ...
            for i in range(num_windows):
                ...
                # PPG 정규화 
                seg_ppg = (seg_ppg - seg_ppg.mean()) / (seg_ppg.std() + 1e-6)
                # 너무 flat하면 pass 
                if np.std(seg_ppg) < 1e-3:
                    continue

                seg_temp = raw_temp[start:end]
                # window단위로 온도 정규화 
                seg_temp = (seg_temp - seg_temp.mean()) / (seg_temp.std() + 1e-6)
