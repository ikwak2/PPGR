...
from torch.nn import functional as F # Softmax 변환용
...
class HybridFusionModel(nn.Module):
    def __init__(self, num_classes=16):
        ...
        ### Backbone 레이어 Freeze (가중치 고정) ###
        for param in self.ppg_backbone.parameters():
...
def compute_eer(labels, probs):
    """
    다중 클래스(Multi-class) 모델에서 One-vs-Rest 방식으로 평균 EER을 계산합니다.
    - labels: 정답 라벨 리스트 (N,)
    - probs: 모델의 예측 확률 (N, Num_Classes) - Softmax 통과 값
    """
    n_classes = probs.shape[1]
    eer_list = []
    
    # 각 클래스(사용자)를 '본인(Positive)', 나머지를 '공격자(Negative)'로 가정
    for i in range(n_classes):
        # 현재 클래스(i)가 정답이면 1, 아니면 0 (Binary Label 생성)
        y_true = (labels == i).astype(int)
        # 현재 클래스에 대한 모델의 확신도(Probability)
        y_score = probs[:, i]
        ...
        # FAR(fpr)과 FRR(frr)이 교차하는 지점(EER) 찾기
        # |FAR - FRR| 이 가장 작은 지점이 EER 근사치
        idx = np.nanargmin(np.absolute((frr - fpr)))
...
def train_model():
    ...
    for epoch in range(EPOCHS):
        ...
        # ⭐️ EER 계산 호출
        epoch_eer = compute_eer(all_labels, all_probs)
        ...
        history['val_eer'].append(epoch_eer)
        
        print(f"✨ [Epoch {epoch+1}] Train Loss: {epoch_loss:.4f} | Val Loss: {epoch_val_loss:.4f} | Acc: {epoch_acc:.2f}% | EER: {epoch_eer:.4f}")
...
def plot_curves(history, save_path='training_result.png'):
    plt.figure(figsize=(18, 5)) # 가로 길이 늘림
    ...
    # 3. EER Curve (New)
    plt.subplot(1, 3, 3)
    plt.plot(history['val_eer'], label='Val EER', color='red')
    plt.title('EER History (Lower is Better)')
    plt.xlabel('Epochs')
    plt.ylabel('EER')
    plt.legend()
    plt.grid(True)
